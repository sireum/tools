...
class SlangCheckHAMRTest extends TestSuite with TestUtil {
  ...
  def test(projName: String, packageName: String): Unit = {
    ...
    proc"$sireum hamr codegen --no-proyek-ive --package-name $packageName --output-dir $outDir ${json(0)}".echo.console.runCheck()
    ...
    proc"$sireum proyek tipe .".at(outDir).echo.console.runCheck()

    println("Compiling ...")
    proc"$sireum proyek compile .".at(outDir).echo.console.runCheck()
  }
}
