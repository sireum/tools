...
@msig trait RandomLibI {
  ...
  def get_Config_TR_T: Config_TR_T
  def set_Config_TR_T(config: Config_TR_T): RandomLib

  def nextTR_T(): TR_T = {
    var callEnum: ISZ[TR_T_DataTypeId.Type] = ISZ(TR_T_DataTypeId.L_X_Id)

    if(get_Config_TR_T.additiveTypeFiltering) {
       callEnum = get_Config_TR_T.typeFilter
    } else {
       for(h <- get_Config_TR_T.typeFilter) {
         callEnum = ops.ISZOps(callEnum).filter(f => h =!= f)
       }
    }
    ...
    var v: TR_T = c match {
      case TR_T_DataTypeId.L_X_Id => (nextL_X _).apply()
      case _ => halt("Invalid Child")
    }


    if(get_Config_TR_T.attempts >= 0) {
     for(i <- 0 to get_Config_TR_T.attempts) {
       if(get_Config_TR_T.filter(v)) {
        return v
       }
       if (get_Config_TR_T.verbose) {
         println(s"Retrying for failing value: $v")
       }
       c = callEnum(gen.nextZBetween(0, callEnum.size-1))
  ...
  def get_Config_L_X: Config_L_X
  def set_Config_L_X(config: Config_L_X): RandomLib

  def nextL_X(): L_X = {
    var value1: Z = nextZ()

    var v: L_X = L_X(value1)

    if(get_Config_L_X.attempts >= 0) {
     for(i <- 0 to get_Config_L_X.attempts) {
        if(get_Config_L_X.filter(v)) {
          return v
        }
        if (get_Config_L_X.verbose) {
          println(s"Retrying for failing value: $v")
        }
        value1 = nextZ()
        v = L_X(value1)
...
@record class RandomLib(val gen: org.sireum.Random.Gen) extends RandomLibI {
  ...
  // ============= TR_T ===================
  def alwaysTrue_TR_T(v: TR_T): B = {return T}

  var config_TR_T: Config_TR_T = Config_TR_T(100, _verbose, F, ISZ(), alwaysTrue_TR_T _)

  def get_Config_TR_T: Config_TR_T = {return config_TR_T}
  ...
  // ============= L_X ===================
  def alwaysTrue_L_X(v: L_X): B = {return T}

  var config_L_X: Config_L_X = Config_L_X(100, _verbose, alwaysTrue_L_X _)

  def get_Config_L_X: Config_L_X = {return config_L_X}
