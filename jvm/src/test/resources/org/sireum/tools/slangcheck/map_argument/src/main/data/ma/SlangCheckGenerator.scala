...
@record class Gen_MapZString(param: RandomLibI) extends MJen[Map[Z, String]] {
  override def generate(f: Map[Z, String] => Jen.Action): Jen.Action = {
    var continue = Jen.Continue
    while (T) {

      continue = f(param.nextMapZString())
