...
@record class Gen_MSU32U8(param: RandomLibI) extends MJen[MS[U32, U8]] {
  override def generate(f: MS[U32, U8] => Jen.Action): Jen.Action = {
    var continue = Jen.Continue
    while (T) {

      continue = f(param.nextMSU32U8())
...
@record class Gen_testThing(param: RandomLibI) extends MJen[testThing] {
  override def generate(f: testThing => Jen.Action): Jen.Action = {
    var continue = Jen.Continue
    while (T) {

      continue = f(param.nexttestThing())
