...
@msig trait RandomLibI {
  ...
  def nextISZexample(): ISZ[example] = {
     val length: Z = gen.nextZBetween(0, 256)
        var temp: ISZ[example] = ISZ()
        for (r <- 0 until length) {
          temp = temp :+ nextexample()
        }
  ...
  def nextOptionexample(): Option[example] = {
    val none: Z = gen.nextZBetween(0,1)

    if(none == 0)
     return Some(nextexample())
    else
     return None()
  }
}
