...
@msig trait RandomLibI {
  ...
  def nextc_AZ(): c_A[Z] = {
    assert(F, "nextc_AZ needs to be implemented")
    halt("nextc_AZ needs to be implemented")
  }
...
@record class RandomLib(val gen: org.sireum.Random.Gen) extends RandomLibI {
  ...
  // ============= c_A[Z] ===================
  def alwaysTrue_c_AZ(v: c_A[Z]): B = {return T}

  var config_c_AZ: Config_c_AZ = Config_c_AZ(0, 20, 100, _verbose, alwaysTrue_c_AZ _)
  def get_Config_c_AZ: Config_c_AZ = {return config_c_AZ}

  def set_Config_c_AZ(config: Config_c_AZ): RandomLib ={
    config_c_AZ = config
    return this
  }
