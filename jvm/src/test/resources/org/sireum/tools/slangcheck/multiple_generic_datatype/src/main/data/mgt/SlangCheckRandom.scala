...
@msig trait RandomLibI {
  ...
  def nextc_AZBC(): c_A[Z, B, C] = {
    assert(F, "nextc_AZBC needs to be implemented")
    halt("nextc_AZBC needs to be implemented")
  }
...
@record class RandomLib(val gen: org.sireum.Random.Gen) extends RandomLibI {
  ...
  // ============= c_A[Z] ===================
  def alwaysTrue_c_AZBC(v: c_A[Z, B, C]): B = {return T}

  var config_c_AZBC: Config_c_AZBC = Config_c_AZBC(0, 20, 100, _verbose, alwaysTrue_c_AZBC _)
  def get_Config_c_AZBC: Config_c_AZBC = {return config_c_AZBC}

  def set_Config_c_AZBC(config: Config_c_AZBC): RandomLib ={
    config_c_AZBC = config
    return this
  }
